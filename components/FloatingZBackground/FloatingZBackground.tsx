...
export default function FloatingZBackground() {
  ...
  return (
    <>
      {lettersData.map(({ key, size, rotate, left, delay, duration }) => (
        <span
          key={key}
      ...
      <style jsx>{`
        @keyframes rise-parallax {
          ...
          10% { opacity: 0.3; }
          40% { transform: translateY(-40vh) translateX(5px) rotate(10deg); opacity: 0.6; }
          70% { transform: translateY(-70vh) translateX(-5px) rotate(-10deg); opacity: 0.4; }
          100% { transform: translateY(-120vh) translateX(0) rotate(20deg); opacity: 0; }
        }
